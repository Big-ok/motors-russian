...
form.addEventListener('submit', (e) => { // Отлавливаем событие нажатие на кнопку отправить
    e.preventDefault();
    ...
    for (let i = 0; i < formElements.length; i++) {
        ...
        if (elementClass.indexOf('validate') + 1) { // Оставляем элементы которые имеют класс validate'
            ...
            if (formElements[i].type === 'checkbox' && formElements[i].checked === false) { // Проверяем заполнено ли поле checkbox
                ...
                formElements[i].parentElement.classList.add('error'); // Добавляем родителю класс error
                formElements[i].parentElement.insertAdjacentHTML('beforeend', '<span class="helper-text">Это поле обязательно для заполнения</span>');
            } else {
                l++;
            }
    ...
    if (l === s) {
        ...
        fetch('https://api.telegram.org/bot1801012612:AAHl0LxzdBxyCtpQSVmVmGQ3T4dqHhAx49U/sendMessage?chat_id=-584993655&text=Сообщение с сайта: ' + hostname + ', от пользователя: ' + name + ', тел: ' + tel)
            ...
            .then(i => {
                if (i.ok) {
                    ...
                    form.insertAdjacentHTML('beforebegin', '<div class="sending">Спасибо, ваше сообщение отправлено!</div>');
